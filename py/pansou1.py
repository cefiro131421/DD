...
class CloudSearchSpider(Spider):
    ...
    DEFAULT_BASE_URL = "https://tgsou.252035.xyz"
    HEADERS = {
    ...
    def __init__(self):
        ...
        retries = Retry(total=self.MAX_RETRIES, backoff_factor=self.BACKOFF_FACTOR, status_forcelist=[502, 503, 504])
    ...
    def _parse_and_sort_results(self, data):
        enabled_pan_types = [pan.strip() for pan in self.pan_priority.split(',') if pan.strip()] if self.pan_priority else []
        all_items = []
        ...
        for cloud_type, items in data.get('data', {}).get('merged_by_type', {}).items():
            ...
            for item in items:
                ...
                remarks = f"{time_str}|{source}" if time_str else source
